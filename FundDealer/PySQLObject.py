...
"""PySQLObject
A database wrapper which provides Python syntax for SQL operations.

Copyright (C) 2004 Scott Scriven
    ( http://toykeeper.net/ )
...
class db:
    ...
    def query(self, q, data=None):
        try:
            ...
            c.execute(q,data)
            return c
        except dbmod.Error, e:
            print 'Something wrong with query: %s' % q 
            print '%s' % e 
            raise
